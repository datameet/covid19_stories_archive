 ...
 * JS for Hoopz Ad.
 */
var ad_received = false;
(function($) {
  Drupal.behaviors.esad = {
    attach: function(context, settings) {
      $(document).ready(function() {
	var cur_loc = $(location).attr('pathname');
        var n = cur_loc.search("siteadmin");
        if (n == -1 ) {
          if (Drupal.settings.esad) {
              if (!ad_received) {
              pullAd();
            }
            ad_received = true;
      ...
      function pullAd() {
        var div_arr = Drupal.settings.esad;
        var len = div_arr.length;
        for (var i = 0; i < len; i++) {
            var script = document.createElement('script');
            script.innerHTML = div_arr[i][1];
            document.getElementById(div_arr[i][0]).appendChild(script);
        }
      }
    }
  };
