...
jQuery(document).ready(function() {
    ...
    function fontIncrease(p1, p2) {
        curSize = parseInt(jQuery(p1).css('font-size')) + 1;
        if (curSize <= p2)
            jQuery(p1).css('font-size', curSize);
    }

    function fontDecrease(p1, p2) {
        curSize = parseInt(jQuery(p1).css('font-size')) - 1;
        if (curSize >= p2)
            jQuery(p1).css('font-size', curSize);
    }
    ...
    jQuery('#incfont').click(function() {
        fontIncrease('.detail_title', 30);
        fontIncrease('.summary_height p', 20);
        fontIncrease('.news_summary p', 20);
        fontIncrease('.new-detailed-news-content>.field-name-body>.field-items>.field-item p', 20);
    });
    jQuery('#decfont').click(function() {

        fontDecrease('.detail_title', 20);
        fontDecrease('.summary_height p', 10);
        fontDecrease('.news_summary p', 10);
        fontDecrease('.new-detailed-news-content>.field-name-body>.field-items>.field-item p', 10);
...
function clickTrack(ec, ea, el) {
    ga('esakalTracker.send', 'event', {
        eventCategory: ec,
        eventAction: ea,
        eventLabel: el
    });
...
function calcHeights() {
    bannerImage = jQuery('.banner-image').height();
    topMenu = jQuery('.navbar.navbar-default.top-header.container').height();
}

function adjustAdversitements() {
    var scroll = jQuery(window).scrollTop();
    if (scroll > (bannerImage)) {
        var topValue = topMenu + 'px';
        jQuery('.fixed-right-jacket').css({ 'top': topValue, 'position': 'fixed' });
        jQuery('.fixed-left-jacket').css({ 'top': topValue, 'position': 'fixed' });

    } else {
        var topValue = topMenu + bannerImage - 6;
        topValue = topValue + 'px';
        jQuery('.fixed-right-jacket').css({ 'top': topValue, 'position': 'absolute' });
        jQuery('.fixed-left-jacket').css({ 'top': topValue, 'position': 'absolute' });
    }
}
...
var lastScrollTop = 0;
var st = null;
jQuery('#nav-top-ad').css({ "z-index": "0" });

window.onscroll = function() { myFunction() };

var screen_width = null;
var header_offsettop = jQuery('#pageHeader').offset().top;

function myFunction() {
    screen_width = document.documentElement.clientWidth;
    if (window.pageYOffset > 180) {
        jQuery('#pageWrapper .scrollable-navigation-bar').css({ "z-index": "0" });
        jQuery("#pageWrapper").css({ "position": "sticky", "top": "0px", "z-index": "100", "left": "-1px", "width": "101%" });


        if (screen_width < 767) {
            jQuery('#pageHeader').css({ 'display': 'none' });
            jQuery("#pageWrapper").css({ "position": "fixed" });
            jQuery('#pageWrapper #pageSubHeader #mainNav').css({ "z-index": "100" });
        }
        st = jQuery(this).scrollTop();
        if (st > lastScrollTop) {
            if (screen_width < 767) {
                jQuery('#pageHeader').css({ "display": "none" });
                jQuery('#pageWrapper #pageSubHeader').css({ "height": "0" });
                jQuery('#pageWrapper #pageSubHeader .social-icon').css({ "display": "none" });
                jQuery('#pageWrapper .siteSearch').css({ "display": "none" });
                jQuery('#pageWrapper .voice-search').css({ "display": "none" });
            } else {
                jQuery('#pageHeader').css({ 'display': 'none' });
            }
            ...
            if (screen_width < 767) {
                jQuery('#pageHeader').css({ "display": "flex" });
                jQuery('#pageWrapper #pageSubHeader .social-icon').css({ "display": "flex" });
                jQuery('#pageWrapper .siteSearch').css({ "display": "flex" });
                jQuery('#pageWrapper .voice-search').css({ "display": "flex" });
                jQuery('#pageWrapper #pageSubHeader').css({ "height": "unset" });
            } else {
                //jQuery('#pageHeader').css({ 'display': 'flex' });
            }
        }
    ...
    if (window.pageYOffset == 0) {
        jQuery("#pageWrapper").css({ "position": "unset", "top": "0px", "z-index": "100", "left": "0px", "width": "100%" });
        // jQuery('#nav-top-ad').css({ "display": "unset" });
        jQuery('#pageHeader').css({ "display": "flex" });
        jQuery('#pageWrapper #pageSubHeader .social-icon').css({ "display": "flex" });
        jQuery('#pageWrapper .siteSearch').css({ "display": "flex" });
        jQuery('#pageWrapper .voice-search').css({ "display": "flex" });
